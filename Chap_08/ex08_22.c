...
char grade_fun(int jumsu); // 입력한 점수의 등급을 계산하는 함수
char alpa_err(char op); // Y, N 외 알파벳 입력하면 메시지 출력하는 함수
void jumsu_err(int jumsu); // 0~100 이외 숫자 입력하면 메시지 출력하는 함수
void end_fun(char op); // main( ) 함수에서 N을 입력받으면 프로그램 종료
